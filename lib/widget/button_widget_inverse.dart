...
class AppButtonInverse extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(// Use provided width or default to double.infinity
      height: height ?? 55.h, // Use provided height or default to 50.h
      child: MaterialButton(
        elevation: elevation ?? 0.0,
        padding: EdgeInsets.symmetric(vertical: padding ?? 10.0,horizontal: 45).r,
        onPressed: callback,
        color: buttonColor ?? AppColor.primaryColor, // Use custom color or default to primary color
        shape: RoundedRectangleBorder(
          side: BorderSide(color: borderColor ?? AppColor.primaryColor), // Match the border color to the button color
          borderRadius: BorderRadius.all(Radius.circular(radius ?? kBorderRadius)),
        ),
        child: Text(
          label,
          style: textBold.copyWith(
            color: textColor ?? AppColor.white, // Use custom text color or default to white
            fontSize: 16.spMin,
          ),
        ),
      ),
